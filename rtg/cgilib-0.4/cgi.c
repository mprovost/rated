...
s_cgi **cgiInit ()
{
	...
	if (esp && strlen(esp)) {
		...
		return NULL;
	    snprintf (line, strlen(esp)+2, "%s", esp);
	} else
	    return NULL;
    } else {
	length = 0;
	...
	for (cp = fgets(tmp, 100, stdin); cp != NULL;
		...
		if (line) {
			...
			line[strlen(line)-1] = '&';
		    strncpy(ip, line, length * sizeof(char));
		}
		ip = strncat(ip, tmp, length * sizeof(char));
		if (line)
		    free (line);
		line = ip;
	    }
	...
	if (i<numargs) {
		...
		snprintf (sptr, (strlen(result[k]->value)+(ip-esp)+2)*sizeof(char), "%s\n", result[k]->value);
		cp = ++esp;
